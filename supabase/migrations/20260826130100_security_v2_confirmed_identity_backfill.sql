  ...
  if v_profile_count > 0 then
    ...
    from (select company_id, string_agg(id::text, ',' order by id) user_ids from public.profiles
      where company_id is not null
        and company_id <> '8ce09e9b-a061-4a8b-b5f3-d286cc18be8a'::uuid
      group by company_id having count(*) > 1) candidates;
...
-- Human-confirmed on 2026-08-26: both individuals intentionally own Wreach.
-- Do not reduce either membership without separate explicit human approval.
insert into public.company_memberships (company_id, user_id, role, status)
select '8ce09e9b-a061-4a8b-b5f3-d286cc18be8a'::uuid, u.id, 'owner', 'active'
from auth.users u
where u.id in (
  '220d30ce-2286-4ec7-b17b-5d871f9fa038'::uuid,
  '9e62a891-3805-422d-8f41-e65ce69d5a4b'::uuid
)
