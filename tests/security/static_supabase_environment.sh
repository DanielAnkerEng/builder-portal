...
client="$repo_dir/js/supabase.js"; local_config="$repo_dir/js/supabase.local.js"; public_config="$repo_dir/js/supabase-public-config.mjs"
rg -q "https://oqwpfnmqeriupujpssxz\.supabase\.co" "$public_config"
...
if rg -n "($secret_prefix|$service_key|$secret_key)" "$client" "$local_config" "$public_config"; then echo 'Privileged key found in frontend config' >&2; exit 1; fi
